...
# Scaled for comma 3x larger display (approx 2.25x from mici)
# Original mici value: 12.7
TORQUE_ANGLE_SPAN = 25.0  # degrees - wider arc for larger display

DEBUG = False


def blend_colors(a: rl.Color, b: rl.Color, f: float) -> rl.Color:
  """Blend two colors in HSV space for smooth transitions."""
  f = max(0.0, min(1.0, f))  # clamp f to [0, 1]
...
def quantized_lru_cache(maxsize=128):
  """LRU cache with quantized inputs for smoother rendering."""
  def decorator(func):
    cache = OrderedDict()
    @wraps(func)
    def wrapper(cx, cy, r_mid, thickness, a0_deg, a1_deg, **kwargs):
      ...
      key = (round(cx), round(cy), round(r_mid),
             ...
             round(a0_deg * 10) / 10,    # 0.1 degree precision for smoother angle transitions
             round(a1_deg * 10) / 10,
...
def arc_bar_pts(cx: float, cy: float,
  ...
  def get_cap(left: bool, a_deg: float):
    ...
    # sweep 90 degrees in the local (t,n) frame: from outer edge toward inside
    if not left:
...
class TorqueBar(Widget):
  def __init__(self, demo: bool = False):
    super().__init__()
    self._demo = demo
    self._torque_filter = FirstOrderFilter(0, 0.1, 1 / DEFAULT_FPS)
    self._torque_line_alpha_filter = FirstOrderFilter(0.0, 0.1, 1 / DEFAULT_FPS)
  ...
  def _render(self, rect: rl.Rectangle) -> None:
    # Scaled dimensions for comma 3x (approx 2.25x from mici)
    # Original mici values: offset 22-26, height 14-56
    # Increased offset to move the bar up and prevent clipping
    torque_line_offset = np.interp(abs(self._torque_filter.x), [0.5, 1], [120, 140])
    torque_line_height = np.interp(abs(self._torque_filter.x), [0.5, 1], [32, 126])
    ...
    # Scaled radius for comma 3x (original mici: 1200)
    torque_line_radius = 2700
    top_angle = -90
    ...
    cx = rect.x + rect.width / 2  # center of screen
    ...
    # draw gradient from center to 65% of the bg torque bar width
    ...
    # v0.10.0 shader_polygon uses dict-based gradients
    gradient = {
    ...
    if abs(self._torque_filter.x) < 0.5:
      ...
      # Scaled dot radius for comma 3x (original mici: 10 // 2 = 5)
      rl.draw_circle(int(cx), int(dot_y), 11,
